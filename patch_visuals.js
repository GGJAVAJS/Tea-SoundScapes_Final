...
const hbStart = code.indexOf('function CarsHeartbeatPulses({ dbLevel }');
const hbEnd = code.indexOf('function CarsGuardianBackground');

if (hbStart !== -1 && hbEnd !== -1) {
  const newHb = `function CarsHeartbeatPulses({ dbLevel }: { dbLevel: number }) {
...
`;
  code = code.substring(0, hbStart) + newHb + code.substring(hbEnd);
}

const bgStart = code.indexOf('function CarsGuardianBackground({ dbLevel }');
const bgEnd = code.indexOf('export function GuardianView');
...
`;
  code = code.substring(0, bgStart) + newBg + code.substring(bgEnd);
}
