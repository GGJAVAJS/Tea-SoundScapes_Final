...
// The bottom of the file is broken.
// I'll strip everything from `function ReportCard` downwards, and rewrite it.
const stripIdx = code.indexOf('function ReportCard');
if (stripIdx !== -1) {
  code = code.substring(0, stripIdx);
}
...
fs.writeFileSync('src/views/DiaryView.tsx', code + append);
console.log('Success Relatorios Rewrite');
