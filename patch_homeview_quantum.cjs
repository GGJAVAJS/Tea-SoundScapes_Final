...
// Ensure import for GenerativeArtSceneV3
if (!code.includes('GenerativeArtSceneV3')) {
    code = code.replace(/import SoundNebulaShader from '\.\.\/components\/ui\/SoundNebulaShader';/, "import SoundNebulaShader from '../components/ui/SoundNebulaShader';\nimport GenerativeArtSceneV3 from '../components/ui/quantum-nebula';");
}

// Replace MixerOverlay completely
const overlayCode = `function MixerOverlay({ dominantSoundId, dominantLabel, DominantIcon, activeSoundIds, allMixerSounds, volumes, handleVolumeChange, setIsMixerOpen, isGlobalPause, setIsGlobalPause }) {
  ...
  return (
    ...
    >
       ...
       <div className="relative z-10 flex flex-col h-full w-full">
         ...
         {/* Spacer to push volumes to the bottom */}
         <div className="flex-1" />
...
if (start !== -1 && end !== -1) {
  code = code.substring(0, start) + overlayCode + '\n' + code.substring(end);
  fs.writeFileSync('src/views/HomeView.tsx', code);
  console.log("Successfully updated HomeView with Quantum Nebula.");
} else {
