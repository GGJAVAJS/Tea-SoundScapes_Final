...
export default function ShaderDemo_ATC(){
  ...
  useEffect(() => {
    ...
    const resize = () => {
      // Optimizacao para mobile: Pixel ratio 1 ou no maximo 1.5
  ...
  return (
    <div style={{position:"absolute", inset: 0}}>
      ...
      <pre ref={preRef} style={{position:"absolute",top:8,left:8,color:"#0f0",whiteSpace:"pre-wrap",zIndex:50}}/>
    </div>
  )
}
`;

fs.writeFileSync('src/components/ui/atc-shader.tsx', code);
