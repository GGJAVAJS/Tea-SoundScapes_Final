...
code = code.replace(
  /onClose=\{\(\) => \{\s*setIsRefugeOpen\(false\);\s*if \(themeMode === 'child' && childAutonomyFilter\) \{\s*setIsPostCrisisDiaryOpen\(true\);\s*\} else if \(themeMode === 'child' && !childAutonomyFilter\) \{[\s\S]*?\} catch\(e\) \{\}\s*\}\s*\}\}/,
  `onClose={() => {
