...
#define F +vec4(texture(iChannel0,.3+p.xz*s/3e3).x)/(s+=s) 

void mainImage( out vec4 c, vec2 w )
{
    vec2 uv = w / iResolution.xy - 0.5;
    
    // Mobile aspect ratio fix
    uv.x *= iResolution.x / iResolution.y;
    
    vec4 p = vec4(uv, 1.0, 1.0), d = p, t;
    p.z += iTime * 20.;
    d.y -= .4;
...
export default function NatureLandscapeShader() {
  ...
  useEffect(() => {
    ...
    // Create a pure grayscale noise texture (value noise like Shadertoy's iChannel0)
